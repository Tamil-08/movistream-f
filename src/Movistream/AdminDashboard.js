...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        await api.get("/get").then((res) => setInquiries(res.data));

        await api.get("/getal").then((res) => setUsers(res.data));
      } catch (err) {
        console.log(err);
      } finally {
        setReload(false);
  ...
  const handleDelete = async (type, userId, itemId) => {
    if (type) {
      try {
        await api.delete(`/deleteid/${userId}`).then(() => {
          alert("User Deleted.");
          setReload(true);
        });
      } catch (err) {
                            ...
                            <tr key={idx}>
                              ...
                              <td>
                                <button
                                  ...
                                  onClick={() =>
                                    handleDelete(true, user.id, null)
                                  }
                                >
                                  Delete
                                </button>
        ...
        {editingIndex !== null ? 'Update' : 'Create'}
            ...
            <table className="admin-table-glass">
              ...
              <button className="delete-btn" onClick={() => handleDelete(idx)}>Delete</button> */}
                      <button
                        className="edit-btn"
                        onClick={() => handleEdit(inq, inq.id)}
                      >
                        Edit
                      </button>
                      <button
                        className="delete-btn"
                        onClick={() => handleDelete(false, null, inq.id)}
                      >
                        Delete
                      </button>
                    </td>
